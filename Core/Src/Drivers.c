...
void Ini_LCD(void)
	...
	if(Flg_Display == true)						// Si tengo display lo habilito
		...
		while (Time_Out_disp > 0);				// Esta primera demora es para el power on
		...
		cmd_ini(Func_set2);         			// Sistem Set, 8 Bits;2lineas;5x10dots
		Time_Out_disp = 10;						// demora > 10 ms
		while(status_display());				// Espera a que este disponible

		cmd_ini(Func_set2);         			// Sistem Set, 8 Bits;2lineas;5x10dots
		Time_Out_disp = 10;						// demora > 10 ms
		while(status_display());				// Espera a que este disponible  demora > 50 ms
