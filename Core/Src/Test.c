...
extern void Estado_Tara(void);               // Respuesta sobre el estado de la toma de taras a la consulta que envia la PC cada 1 seg
    ...
    while (Flag_Trabajo == True) // sin un comando que lo cambie no salgo del Modo TEST
        ...
        if (Comunica_PC.flg_rx == true) // Si la recepcion fue correcta => se ejecuta el comando recibido
            ...
            case V_Cuentas: // Ver las cuentas del A/D
                ...
                Ver_Cuentas();
                break;
            case T_Taras: // T_Taras = 5 (0x05) Tomar las taras
                if (Flag_Errores_Display == false)
                    Print_LCD(11, 3, (int8_t *)"Toma Tara");
                Flag_Enable_Taras = true; // Habilita la toma de taras desde la PC
                Flag_Tara_OK = false;     // Saca el flag indicando que se realizo el proceso de toma de taras
                Flag_Final1_Taras = false;
                Toma_Taras();
                Flag_Parpa = false; // habilita la frecuencia de parpadeo
                Beep();
                TX_Ack(Comu_PC);
                break;
            case T_Taras1:     // T_Taras1 = 84 (0x54) Para ver si termino la tarea de taras
                Estado_Tara(); // Respuesta a la PC sobre la tarea detoma de tara
                break;
            ...
            if (Time_Out_disp <= 1)
                ...
                if ((Flag_Datos_Display == true) && (Flag_Errores_Display == false))
                    {
            ...
            /**************************************************************************************/
            if (Flag_Init_Tara == true) // Pone el flag para habilitar la toma de taras de los platillos porque recibio el comando de la PC
                {
                if (Flag_Enable_Taras == true) // DEBE esperar que pase el plato 1 para comenzar a tomar las taras
                    {
                    if (Flag_Final1_Taras == false)
                        {
                        Flag_Final1_Taras = true;
                    ...
                    if (Flag_Final_Taras == true) // La cantidad de platos va de 0 a n_platillos - 1
                        {
                        ...
                        Flag_Tara_Inicial = true;  // no vuelvo a realizar la tara
                        if (Flag_Errores_Display == false)
                ...
                {
                if (Flag_Enable_Taras == true) // Si esta realizando la tarea de taras y aborta
                    {
                    Flag_Error = false;        // Apaga el Led rojo cada 1 seg para indicar que se esta tomando la tara
                    Flag_Init_Tara = false;    // Pone el flag para deshabilitar la toma de taras de los platillos
                    Flag_Enable_Taras = false; // Deshabilita la toma de taras desde la PC
                    Flag_Tara_OK = True;       // Pone el flag indicando que se realizo el proceso de toma de taras
                    Flag_Error = False;        // Apaga el Led rojo indicando que finalizo el proceso de toma de tara
                    if (Flag_Errores_Display == false)
                        Print_LCD(11, 3, (int8_t *)"Aborta   ");
                    }
                }
            /**************************************************************************************/
...
void Get_Nserie_AD(void)
    ...
    for (j = 0; j < 5; j++)
        {
        Comunica_PC.buf_tx_PC[7 + j] = Config.n_serie[2 + j]; // le sumo 2 porque el número de serie es de 5 dígitos no de 7
        }
...
void Placa_salida(void)
    ...
    Time_Out_G = 1000;      // pongo el Time Out en 1000 mseg.
    while (Time_Out_G != 0) // Espera la respuesta de la placa de salida
        ...
        if (Time_Out_G <= 1)
            {
            if (Flag_Errores_Display == false)
                Print_LCD(0, 3, (int8_t *)"Error P/salida respu");
            fatal_error_Beep();
            TX_Nak(Comu_PC);
            }
        }
...
void Calculo_Velocidad(void)
    ...
    if (Time_Out_Velo <= 1)
        ...
        if (Conta_Velo_Maq != 0) // Maquina funcionando
            ...
            Dato_Velo[3] = Dato_Velo[2]; // arreglo para poner el punto decimal
            Dato_Velo[2] = '.';
            Dato_Velo[4] = Null;
            Cont_Velo = 0;
            Conta_Velo_Maq = 0;
