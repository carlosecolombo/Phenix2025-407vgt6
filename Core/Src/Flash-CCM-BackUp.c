 ...
 *  			  la copia se realiza no por nombre de la  variable sino en forma masiva
 *  Retorna		:
 */
...
void Init_Variables_Config(void)
    ...
    if (Firma_FL != Firma_Def) // Si la Firma no es igual a la definida en FLASH
        ...
        {
        /* Flash siempre es la base para Config/Comunicaciones/Coeficientes */
    ...
    Time_Out_G = 2000; // pongo el Time Out en 2 seg; // Espero para ver
    ...
    /*		INICIALIZO las taras con el cero de calibración    */
    for (i = 0; i < 8; i++)
        {
        for (j = 0; j < 500; j++)
            {
 ...
 *  Objetivo 	: copio los datos de Flash a la Ram CCM
 *  Entrada  	:
 *  Descripcion 	:
 *
 *************************************************************************/
void Grabo_FL_to_CCM(void)
    ...
    for (y = 0; y < MaxLineas; y++)
        ...
        for (j = 0; j < Max_Tamanos; j++)
            {
            Tabla_Limites_CCM[j][y] = Tabla_Limites_FL[j][y]; // le cargo una table cualquiera 0, 30, 60,.....660 gr
            }
        }
...
void Grabo_BackUp_to_CCM(void)
    ...
    /* Habilito acceso a Backup SRAM */
    HAL_PWR_EnableBkUpAccess();
    __HAL_RCC_BKPSRAM_CLK_ENABLE();

    /* Valido firma básica */
    if (Firma_BK != Firma_Def)
        {
        return;
        }

    /* Sincronizo número de grabación en CCM con el de BK */
    Numero_Grabacion_CCM = boot_count_Bk;
    ...
    for (x = 0; x < MaxLineas; x++)
        ...
        for (y = 0; y < Max_Tamanos; y++)
            {
            E_Salidas_CCM[x][y].Cant_Boxes = E_Salidas_BK[x][y].Cant_Boxes;
            E_Salidas_CCM[x][y].Salida_Actual = E_Salidas_BK[x][y].Salida_Actual;
            E_Salidas_CCM[x][y].Puntero = E_Salidas_BK[x][y].Puntero;
            E_Salidas_CCM[x][y].Total_frutos = E_Salidas_BK[x][y].Total_frutos;

            for (j = 0; j < 4; j++)
                {
                E_Salidas_CCM[x][y].Sal[j] = E_Salidas_BK[x][y].Sal[j];
                E_Salidas_CCM[x][y].Tot_Frutos_Box[j] = E_Salidas_BK[x][y].Tot_Frutos_Box[j];
                }
            }
        }
    ...
    for (y = 0; y < Max_Tamanos; y++)
        ...
        for (x = 0; x < MaxLineas; x++)
            {
            Tabla_Limites_CCM[y][x] = Tabla_Limites_BK[y][x];
            }
        }
    }
