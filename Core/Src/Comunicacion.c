...
/*	Fecha inicialización	:	27/05/2022
    Fecha actualización 	:	06/02/2026
...
/*======================================================================================================================
 *
 *  MODULO: Comunicacion.c
 *  PROYECTO: Phenix2025-407VGT6
 *  CPU: STM32F407
 *
 *  DESCRIPCION GENERAL
 *  ------------------
 *  Este módulo implementa la comunicación serie del sistema:
 *
 *    - UART1 : PC <-> CPU (RS-485)
 *    - UART6 : CPU -> Placas de Salida (RS-485)
 *
 *  Se utiliza DMA para transmisión y recepción, junto con detección por IDLE,
 *  watchdogs por software y rutinas de recuperación para garantizar robustez
 *  ante fallos de DMA o estados colgados del periférico.
 *
 *======================================================================================================================
 *
 *  UART1 - PC <-> CPU
 ...
 *
 *      Flujo:
 *        1) Si no hay frame pendiente:
 *             - Copia RxBuffer_PC_Rx -> PC_FrameShadow
 *             - Guarda longitud
 *             - Flag_Rx_It_PC = true
 *        2) Si ya hay frame pendiente:
 *             - Se descarta el nuevo frame
 *             - UART1_cnt_drop++
 *        3) Rearma siempre ReceiveToIdle_DMA.
 *
 *    - El procesamiento real del frame se hace en el lazo principal:
 *        Proc_Rx_PC(RxDataLen_Pc);
 *
 *
 *  TX:
 *    - Modo: DMA
 *    - Función central:
 *        PC_TX_Send_DMA(buf, len)
 *
 *    - Modelo:
 *        * 1 transmisión activa (PC_Tx_Busy)
 *        * 1 mensaje pendiente (PC_Tx_Pending)
 *
 *      Si PC_Tx_Busy == 0:
 *        - Sube DE RS-485
 *        - Espera 1 ms (Time_Out_485)
 *        - HAL_UART_Transmit_DMA()
 *        - Activa watchdog PC_Tx_WD_ms
 *
 *      Si PC_Tx_Busy == 1:
 *        - Guarda último mensaje en Aux_buf_tx
 *        - PC_Tx_Pending = 1
 *
 *    - Callback TX:
 *        HAL_UART_TxCpltCallback()
 *          - Espera breve flag TC
 *          - Baja DE
 *          - PC_Tx_Busy = 0
 *
 *    - Servicio de pendiente (en lazo principal):
 *        PC_TX_Pending_Service();
 *
 *
 *  WATCHDOG UART1 (Timer6 - 1ms):
 *    Si PC_Tx_WD_ms llega a 0 y PC_Tx_Busy sigue activo:
 *        - Se corta DMA
 *        - Se baja DE
 *        - PC_Tx_Busy = 0
 *        - PC_Tx_ForceRecover = 1
 *
 *  Recuperación fuerte (en lazo principal):
 *        PC_TX_ForceRecover_Service()
 *          - HAL_UART_DMAStop()
 *          - Limpia flags ORE/NE/FE/PE/IDLE
 *          - Flush DR
 *          - Rearma ReceiveToIdle_DMA
 *
 *
 *======================================================================================================================
 *
 *  UART6 - CPU -> PLACAS DE SALIDA
 *  ------------------------------
 *
 *  TX:
 *    - Modo: DMA
 ...
 *        - HAL_UART_Transmit_DMA()
 *
 *  WATCHDOG UART6 (Timer6 - 1ms):
 *    Si PSal_Tx_WD_ms llega a 0 y sigue Busy:
 *        - HAL_UART_DMAStop(&huart6)
 *        - huart6.gState = READY
 *        - huart6.TxState = READY
 *        - Baja DE
 *        - PSal_Tx_Busy = false
 *
 *
 ...
 *  ----------------
 *
 *  - Las ISR solo levantan flags.
 *  - Todo el procesamiento pesado se realiza en el lazo principal.
 *  - Se utiliza true = 1 y false = 0.
 *  - La arquitectura está pensada para tolerar fallos de DMA sin requerir reset.
 *
 *======================================================================================================================
 */
...
void PC_TX_Pending_Service(void);
...
void Ini_Serie(int16_t Usart)
    ...
    case 0: // PC
        ...
        huart1.Init.WordLength = Comu_Conf[Usart].bits_data;
    ...
    case 1: // Placas de Salida
        ...
        huart6.Init.WordLength = Comu_Conf[Usart].bits_data;
    ...
    case 2: // ESP8266
        ...
        huart4.Init.WordLength = Comu_Conf[Usart].bits_data;
...
void Proc_Rx_PC(uint16_t LenRx)
    {
#define UART1_RX_CHECKSUM_ENABLE 0 /* 1 = verifica checksum RX, \
                                      0 = no verifica */
    int16_t dato = 0;              // para barrer la recepción
    uint8_t Caracter_rx;
    ...
    while (dato < LenRx)
        ...
        *(Comunica_PC.buf_rx_PC + dato) = PC_FrameShadow[dato]; // RxBuffer_PC_Rx  lo guardo, en la posición que corresponda
        Caracter_rx = PC_FrameShadow[dato];
        if (dato == 0)
            {
            switch (Caracter_rx)
                {
            ...
            if ((dato - 6) >= Long_string)
                {
                    /* verifico si el Check sum es correcto */
#if UART1_RX_CHECKSUM_ENABLE
                if ((Check_sum & 0x00FF) != Caracter_rx)
                    {
                    Comunica_PC.errores_txrx = 5; /* error de checksum -- Recepción */
                    Comunica_PC.flg_rx = False;
                    }
                else
                    {
                    Comunica_PC.flg_rx = True; /* recepción OK */
                    }
#else
                Comunica_PC.flg_rx = True; /* recepción OK */
#endif
...
void Ver_Cpu(void)
    ...
    *(Comunica_PC.buf_tx_PC + 6) = '4';
    strcpy((char *)(Comunica_PC.buf_tx_PC + 7), (const char *)Vers_CPU); // OJO se cargaba buf_rx_PC
 ...
 *
 *  En Usart se debe pasar a que comunicacion se refiere, de acuerdo a la siguiente definicion:
 *  Comu_PC			- 0	-	huart1
 * 	Comu_PSalidas	- 1	-	huart8
 * 	Comu_ESP		- 2	-	huart5
 *
 */
void Transmito_string(int16_t Usart, int8_t Flag_Check_Sun)
...
#define DMA 1 /*   		0 = transmite a demanda, byte a byte \
    ...
    case 0: // Transmito a la PC  por Pooling o DMA
        ...
        Time_Out_485 = 1;                                             /* pongo el Time Out en 1 mseg. */
        while (Time_Out_485 != 0)
            ; /* Espero la 485 */
        ...
        *(Comunica_PC.buf_tx_PC + j + 1) = Null; /* pongo el terminador */
...
#else
        /* DMA: siempre calculo longitud; el checksum solo si corresponde */
        while (*(Comunica_PC.buf_tx_PC + j) != Null)
            {
            if (Flag_Check_Sun == true)
                {
                Check_sum = Check_sum + *(Comunica_PC.buf_tx_PC + j); /* calculo el Check sum */
                }
            j++;
        ...
        if (Flag_Check_Sun == true)
            ...
            *(Comunica_PC.buf_tx_PC + j) = Check_sum; /* Cargo el Check sum en la última posición */
            j++;
            *(Comunica_PC.buf_tx_PC + j) = Null; /* pongo el terminador */
            }

        /* tot_Tx = cantidad de bytes reales a transmitir (sin Null) */
        Comunica_PC.tot_Tx = j;
    ...
    case 2: // Transmito al módulo ECP por Pooling
        while (*(Comunica_ECP.buf_tx_ECP + j) != Null)
            ...
            Check_sum = Check_sum + *(Comunica_ECP.buf_tx_ECP + j); // calculo el Check sum puse el parentesis
            j++;
            }
...
/**---------------------------------------------------------------------------
 * Rutina   	: Transmito_oscilos
 *  Objetivo 	: Realizar tarea de transmitir los datos del osciloscopio a la PC
 *  Entrada  	:
 *  Descripcion 	: Se transmite a la PC el paquete de tamaño fijo del osciloscopio
 *
 *
 */
void Transmito_oscilos()
    {
    uint8_t j;
...
#define DMA 1 /*   	0 = transmite a demanda, byte a byte \
    ...
    while (Time_Out_485 != 0)
        ; /* Espero la 485 */
    while (j <= Frame_Oscilos)
        {
        Tx_char((uint8_t)*(Comunica_PC.buf_tx_PC + j), 0);
        Check_sum = Check_sum + *(Comunica_PC.buf_tx_PC + j); /* calculo el Check sum */
        j++;
        }
    Tx_char((uint8_t)Check_sum, 0);                                 // Transmito el Check sum
    *(Comunica_PC.buf_tx_PC + j) = Check_sum;                       // Cargo el Check sum en la última posición
    *(Comunica_PC.buf_tx_PC + j + 1) = Null;                        /* pongo el terminador */
    HAL_GPIO_WritePin(Dir_485_PC_GPIO_Port, Dir_485_PC_Pin, RESET); /* Pongo la salidas PA10 = 0 (485) */
#else
...
*  Objetivo 	: Muestra los errores  de recepción
*  Entrada  	:
*  Descripcion 	:
*
-----------------------------------------------------------------------------*/
void Error_Handler_UART(void)
    ...
    if (HAL_GPIO_ReadPin(Input8_GPIO_Port, Input8_Pin) == 0) // Verifico el jumper de testeo
        {
        if (Flag_Errores_Display == false)
            {
            Flag_Errores_Display = true;
            Flag_Errores_Display1 = false;
            Clr_LCD(); /* Borro el LCD */
            Print_LCD(0, 0, (int8_t *)" Datos Comunicacion ");
            Print_LCD(0, 1, (int8_t *)"Tran:               ");
            Print_LCD(0, 2, (int8_t *)"Over:     E.Rec:    ");
            Print_LCD(0, 3, (int8_t *)"E.Star:   E.Tou:    ");
            }
        mi_itoa(UART1_cnt_rx_event, (int8_t *)&Dato, 6, false); /* Convierto el INT a ASCII */
        Print_LCD(6, 1, (int8_t *)&Dato);                       // Pongo la cantidad de transmisiones PC

        mi_itoa(Cant_Envios_Sal, (int8_t *)&Dato, 6, false); /* Convierto el INT a ASCII */
        Print_LCD(14, 1, (int8_t *)&Dato);                   // Pongo la cantidad de transmisiones P.Salidas

        mi_itoa(UART1_cnt_err, (int8_t *)&Dato, 3, false); /* Convierto el INT a ASCII */
        Print_LCD(5, 2, (int8_t *)&Dato);                  // Pongo la cantidad de Errores de recepcion

        mi_itoa(UART1_err_ore, (int8_t *)&Dato, 3, false); /* Convierto el INT a ASCII */
        Print_LCD(16, 2, (int8_t *)&Dato);                 // Pongo la cantidad de OverRun de recepcion

        mi_itoa(UART6_cnt_tx_start_fail, (int8_t *)&Dato, 3, false); /* Convierto el INT a ASCII */
        Print_LCD(7, 3, (int8_t *)&Dato);                            // Pongo la cantidad de Errores de Start Uart6

        mi_itoa(UART6_cnt_tx_wd_recover, (int8_t *)&Dato, 3, false); /* Convierto el INT a ASCII */
        Print_LCD(16, 3, (int8_t *)&Dato);                           // Pongo la cantidad de Errores WatchDog Uart6
        }
    else
        {
        if (Flag_Errores_Display1 == false)
            {
            Flag_Errores_Display = false;
            Flag_Errores_Display1 = true;
            Clr_LCD(); /* Borro el LCD */
            Print_LCD(0, 0, (int8_t *)"  Phenix 2025       ");
            Print_LCD(0, 2, (int8_t *)"V.Maq.     Nodo     ");
            Print_LCD(16, 2, (int8_t *)Config.Nodo); // Pongo el Nº de nodo

            if (Flag_Pantalla_Tra == true)
                {
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
    ...
    if (huart->Instance == huart1.Instance)
        {
        /* Recovery automático UART1 RX (DMA + IDLE) */
        UART1_cnt_err++; // Contador de recuperaciones

        /* Capturo diagnóstico antes de limpiar */
        UART1_last_errorcode = huart->ErrorCode;
        UART1_last_sr = huart->Instance->SR;
        ...
        /* Detengo DMA (RX/TX) para recuperar limpio */
        HAL_UART_DMAStop(&huart1);
        HAL_GPIO_WritePin(Dir_485_PC_GPIO_Port, Dir_485_PC_Pin, RESET);
        PC_Tx_Busy = 0;
        PC_Tx_Pending = 0;
        PC_Tx_Pending_Len = 0u;
        PC_Tx_WD_ms = 0;
        ...
        /* Limpio IDLE si quedó colgado (según HAL/serie) */
...
void PC_TX_ForceRecover_Service(void)
    {
    if (PC_Tx_ForceRecover != 0u)
        {
        PC_Tx_ForceRecover = 0u;
        ...
        (void)HAL_UARTEx_ReceiveToIdle_DMA(&huart1, RxBuffer_PC_Rx, Buffer_Rx);
        }
    /* ================= UART1 WD service ================= */
    if (PC_Tx_WD_Expired)
        {
        PC_Tx_WD_Expired = false;
    ...
    /* ================= UART6 WD service ================= */
    if (PSal_Tx_WD_Expired)
        {
        PSal_Tx_WD_Expired = false;

        UART6_cnt_tx_wd_recover++;

        HAL_UART_DMAStop(&huart6);
        huart6.gState = HAL_UART_STATE_READY;
        huart6.RxState = HAL_UART_STATE_READY;

        HAL_GPIO_WritePin(Dir_485_PSalida_GPIO_Port, Dir_485_PSalida_Pin, RESET);
...
/*---------------------------------------------------------------------------*/
/* Rutina    : PC_TX_Pending_Service
 * Objetivo  : Reintentar el envío pendiente (1-slot) en contexto NO-ISR
 * Notas     : Se llama en el loop de Trabajo/Test. "Último gana".
 *---------------------------------------------------------------------------*/
void PC_TX_Pending_Service(void)
    {
    /* Auto-limpieza defensiva */
    if (PC_Tx_Busy && (PC_Tx_WD_ms == 0))
        {
        PC_Tx_Busy = 0;
        }

    if ((PC_Tx_Pending != 0u) && (PC_Tx_Pending_Len > 0u))
        {
        if (PC_Tx_ForceRecover != 0u)
            return;
        ...
        /* Tomo snapshot y limpio pending antes de enviar */
        uint16_t len = PC_Tx_Pending_Len;
        PC_Tx_Pending = 0u;
        PC_Tx_Pending_Len = 0u;

        PC_TX_Send_DMA((uint8_t *)Aux_buf_tx, len);
        }
    }

/*---------------------------------------------------------------------------*/
/*  Bloque      : UART1 (PC) - Modelo de comunicación
 *  Objetivo    : Documentar el flujo RX/TX actual entre PC <-> STM32 por UART1.
 *  Descripcion :
 *    RX (PC -> STM32):
 *      - Se inicializa con:
 *          HAL_UARTEx_ReceiveToIdle_DMA(&huart1, RxBuffer_PC_Rx, Buffer_Rx);
 *
 *      - Callback:
 *          HAL_UARTEx_RxEventCallback(UART1, Size)
 *
 *        Si Flag_Rx_It_PC == TRUE (hay un frame pendiente de procesar):
 *          - Se descarta el nuevo frame (no se pisa el anterior)
 *          - Se incrementa UART1_cnt_drop
 *          - Se rearma ReceiveToIdle_IT para seguir recibiendo
 *
 *        Si Flag_Rx_It_PC == FALSE:
 *          - Se copia RxBuffer_PC_Rx -> PC_FrameShadow
 *          - RxDataLen_Pc = Size
 *          - Flag_Rx_It_PC = TRUE
 *          - Se rearma ReceiveToIdle_IT
 *
 *      - Proceso en loop principal (Trabajo/Test):
 *          - Si Flag_Rx_It_PC == TRUE:
 *              Proc_Rx_PC(RxDataLen_Pc);
 *              (si el frame es OK se setea Comunica_PC.flg_rx = TRUE)
 *              Flag_Rx_It_PC = FALSE;
 *	--------------------------------------------------------------------------------------------
 *    TX (STM32 -> PC):
 *      - Se transmite por DMA (HAL_UART_Transmit_DMA).
 *      - PC_TX_Send_DMA implementa el esquema "Busy + 1 Pending":
 *
 *        Si PC_Tx_Busy == 0:
 *          - PC_Tx_Busy = 1
 *          - Habilita transmisión RS485: DE = 1 (Dir_485_PC SET)
 *          - Arranca DMA: HAL_UART_Transmit_DMA(&huart1, buf, len)
 *          - Si falla el inicio: UART1_cnt_tx_start_fail++, DE=0, Busy=0
 *
 *        Si PC_Tx_Busy == 1:
 *          - Guarda el último mensaje como pendiente:
 *              Aux_buf_tx + PC_Tx_Pending_Len
 *              PC_Tx_Pending = 1
 *
 *      - Callback TX:
 *          HAL_UART_TxCpltCallback(UART1)
 *          - (Opcional) espera flag TC para asegurar fin físico de transmisión
 *          - Baja RS485: DE = 0 (Dir_485_PC RESET)
 *          - PC_Tx_Busy = 0
 *          - Si PC_Tx_Pending == 1:
 *              PC_Tx_Pending = 0
 *              Re-dispara el envío del pendiente
 *
 *  Notas      :
 *    - Este modelo asume "la PC manda 1 paquete y espera respuesta".
 *    - RX descarta frames nuevos si el anterior no fue procesado (back-pressure).
 *    - TX permite 1 envío en curso + 1 mensaje pendiente (se conserva el último).
 */
/*---------------------------------------------------------------------------*/

/*---------------------------------------------------------------------------*/
/* Rutina   	: HAL_UARTEx_RxEventCallback
*  Objetivo 	: Recibe la transmisión realizada por la PC, las placas de salida o ECP, de largo variable
*  Entrada  	:
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
    {
    if (Comm_Lock == true) // si estoy grabando en la flash no hago nada
        return;

    if (huart->Instance == huart1.Instance)
        {
        UART1_cnt_rx_event++; // Contador de eventos

        /* Clamp para no pisar buffers si el HAL devuelve Size > Buffer_Rx */
        uint16_t n = (Size > Buffer_Rx) ? (uint16_t)Buffer_Rx : Size;

        /* Si no hay un frame pendiente, copio a shadow y marco listo para procesar en main */
        if (Flag_Rx_It_PC == false)
            {
            for (uint16_t i = 0; i < n; i++)
                PC_FrameShadow[i] = RxBuffer_PC_Rx[i];
            ...
            {
            /* Ya hay uno pendiente: descarto este (stop&wait debería evitarlo, pero lo contabilizo) */
            UART1_cnt_drop++;
            }

        /* Rearmo RX DMA+IDLE UNA sola vez */
        if (HAL_UARTEx_ReceiveToIdle_DMA(&huart1, RxBuffer_PC_Rx, Buffer_Rx) != HAL_OK)
            {
            UART1_cnt_rx_rearm_fail++;

            /* Intento de recuperación (mínimo) */
            HAL_UART_DMAStop(&huart1);
        ...
        /* Evito interrupción Half-Transfer (no aporta para IDLE y mete jitter) */
        if (huart1.hdmarx != NULL)
            {
            __HAL_DMA_DISABLE_IT(huart1.hdmarx, DMA_IT_HT);
            }
...
/*---------------------------------------------------------------------------*/
/* Rutina   	: PC_TX_Send_DMA
*  Objetivo 	: Transmitir a la PC por DMA
*  Entrada  	: Dato a transmitir y largo del frame
*  Descripcion 	:
*
-----------------------------------------------------------------------------*/
...
static uint8_t PC_TX_Send_DMA(uint8_t *buf, uint16_t len)
    ...
    if (!PC_Tx_Busy)
        ...
        /* Pequeña espera para que la 485 cambie de estado (TIM6 = 1ms) */
        Time_Out_485 = 1u;
        while (Time_Out_485 != 0u)
            { /* espera */
            }

        /* Intento arrancar TX por DMA */
        if (HAL_UART_Transmit_DMA(&huart1, buf, len) != HAL_OK)
            {
            UART1_cnt_tx_start_fail++;

            /* --- RECOVERY FUERTE: evito que el HAL quede en BUSY eterno --- */
            ...
            /* Libero estados propios */
            PC_Tx_Busy = 0u;
            PC_Tx_WD_ms = 0u;

            /* Dejo el mensaje pendiente para reintentar tras recovery */
            ...
            /* Pido recovery completo fuera de la ISR / fuera de esta función */
            PC_Tx_ForceRecover = 1u;
    ...
    /* Si está ocupado: guardo 1 mensaje pendiente (último gana) */
    for (uint16_t i = 0; i < len; i++)
        Aux_buf_tx[i] = buf[i];
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart) /* Transmisión Placa Salidas y ECP */
    ...
    if (huart->Instance == huart1.Instance)
        ...
        /* Libero estados: el pending se enviará en el loop (PC_TX_Pending_Service) */
        PC_Tx_Busy = 0u;
        PC_Tx_WD_ms = 0u;
...
void Transfiero_Salidas(void)
    {
    if (M_Trabajo.Velocidad != 0)
        {
        if (PSal_Tx_Busy == false)
            {
            PSal_Tx_Busy = true;
            PSal_Tx_WD_ms = 30;

            HAL_GPIO_WritePin(Dir_485_PSalida_GPIO_Port, Dir_485_PSalida_Pin, SET);

            Cant_Envios_Sal++;

            if (HAL_UART_Transmit_DMA(&huart6, (uint8_t *)Comunica_PSal.buf_tx_Sal, Comunica_PSal.tot_Tx) != HAL_OK)
                {
                PSal_Tx_Busy = false;
                PSal_Tx_WD_ms = 0;
                UART6_cnt_tx_start_fail++;
...
void Asig_Salidas(void)
    ...
    uint8_t k;       /* Lazo de borado de Tot_Frut_Salidass */
    ...
    for (j = 0; j < 4; j++)
        ...
        if (j < Cant)
            ...
            E_Salidas_CCM[x][y].Tot_Frutos_Box[j] = 0;
            }
    ...
    if (Borrar == 1)
        ...
        E_Salidas_CCM[x][y].Salida_Actual = E_Salidas_CCM[x][y].Sal[0]; /* actualizo salida actual */

        for (k = 0; k < Config.n_salidas; k++)
            Tot_Frut_Salidas[k] = 0;
        }
    else
        {
        if (Flag_Errores_Display == false)
    ...
    E_Salidas_CCM[x][0].Puntero = 0;       /* inicializo el tamaño 0 */
                                           /*
                                               if (((x + 1) >= Config.n_lineas))      								// cuando llego a la última línea guardo los datos en la RAM BACKUP
                                                   {
                                                   if (((y + 1) >= Max_Tamanos - 1))
                                                           {
                                                       Flg_Write = true;   										// Pongo el flag para grabar en la BackUp ram
                                                       Flg_WriteFl = true; 										// Pongo el flag para grabar en la flash
                                                           }
                                                   }
                                           */

    if ((x * y) >= 160) // hay que probar
        {
        Flg_Write = true;   // Pongo el flag para grabar en la BackUp ram
        Flg_WriteFl = true; // Pongo el flag para grabar en la flash
        }
...
void Asig_Etiqueta(void)
    ...
    if ((x * y) >= 80) // cuando llego a la última línea guardo los datos en la RAM BACKUP
        {
        Flg_Write = true;   // Pongo el flag para grabar en la BackUp ram
        Flg_WriteFl = true; // Pongo el flag para grabar en la flash
        }
